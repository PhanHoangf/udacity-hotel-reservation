...
public class ReservationService {
    ...
    public Reservation reserveARoom(Customer customer, IRoom room, Date checkInDate, Date checkOutDate) {
        ...
        List<Reservation> bookedRoom = reservations
                .stream()
                .filter(reservation1 -> Objects.equals(reservation1.getRoom().getRoomNumber(), room.getRoomNumber())).toList();
    ...
    private List<String> getUnavailableRoomsNumber(Date checkInDate, Date checkOutDate) {
        ...
        for (Reservation re : reservations) {
            if (re.getCheckInDate().before(checkInDate) && re.getCheckOutDate().after(checkOutDate)
                    ...
                    || re.getCheckInDate().equals(checkInDate) && re.getCheckOutDate().equals(checkOutDate)
            ) {
